...
- (void)spawnPlayerLasers
{
  _playerLasers = [NSMutableArray arrayWithCapacity:100];
  for (int i = 0; i < 100; ++i) {
    Laser *laser = [[Laser alloc] initWithLaserType:LaserTypePlayer];
    laser.name = @"playerLaser";
    laser.hidden = YES;
    [_playerLasers addObject:laser];
...
- (void)spawnPlayerLaser
{
  ...
  int i;
  for (i = 0; i < 100; ++i) {
    laser = _playerLasers[i];
    if (laser.hidden) break;
  }
  laser.hidden = NO;
